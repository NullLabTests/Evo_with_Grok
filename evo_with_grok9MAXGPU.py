...
class DQNAgent:
    ...
    def act(self, state):
        ...
        logging.info(f"State tensor device: {state.device}")
        with torch.no_grad():
    ...
    def replay(self, batch_size):
        ...
        for state, action, reward, next_state, done in minibatch:
            ...
            logging.info(f"State tensor device in replay: {state.device}")
            target = reward
            if not done:
...
def load_checkpoint(agent):
    ...
    try:
        agent.model.load_state_dict(torch.load("dqn_model.pt", weights_only=True))
        agent.target_model.load_state_dict(torch.load("dqn_model.pt", weights_only=True))
...
try:
    while True:
        ...
        agent.replay(batch_size=128)  # Increased batch size
        if iteration % 100 == 0:
