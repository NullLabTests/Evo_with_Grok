...
def tentacle(statement):
    ...
    It uses a simplified approach to check for logical validity.
    ...
    statement = ''.join(char for char in statement if char.isalnum() or char.isspace()).strip()
    ...
    # If not in equivalences, attempt to evaluate the statement
    try:
        # Replace logical operators with Python equivalents
        expression = statement.replace("implies", "->").replace("and", "and").replace("or", "or").replace("not", "not")
        
        # Define a simple implication function
        def implies(a, b):
            return not a or b
        
        # Evaluate the expression using eval
        result = eval(expression, {"implies": implies, "a": True, "b": True, "c": True})
        
        # If the result is True, the statement is valid
        return bool(result)
    except:
...
if __name__ == "__main__":
    ...
    print(tentacle('A implies B and B implies A'))  # Should print: True
