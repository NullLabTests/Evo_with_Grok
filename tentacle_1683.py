...
def tentacle(statement):
    ...
    This function evaluates logical statements by checking against a set of known logical equivalences.
    It also allows for custom expressions to be evaluated using Python's eval function.
    
    Args:
    statement (str): A string containing a logical statement or a mathematical expression.
    
    Returns:
    bool or str: True if the statement is logically valid, False if it's not, or a string result for custom expressions.
    ...
    # If not a known logical equivalence, try to evaluate it as a mathematical expression
    try:
        # Evaluate the expression and convert the result to a string
        result = str(eval(statement))
        return result
    except Exception as e:
        # If evaluation fails, return False as it's not a valid logical statement or expression
        return False
...
if __name__ == "__main__":
    ...
    print(tentacle('x + y'))  # Should print: False (as x and y are undefined)