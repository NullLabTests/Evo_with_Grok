...
def tentacle(statement):
    ...
    # If not in the list of equivalences, attempt to evaluate the statement
    try:
        # Convert logical operators to Python equivalents
        python_expr = statement.replace("implies", "->").replace("and", "and").replace("or", "or").replace("not", "not").replace("is equivalent to", "==")
        
        # Define helper function for implication
        def implies(a, b):
            return not a or b
        
        # Evaluate the expression
        result = eval(python_expr, {"implies": implies, "->": implies})
