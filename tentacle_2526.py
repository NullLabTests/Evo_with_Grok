...
def tentacle(statement):
    ...
    and transitivity of implication.
    ...
    # Check for simple mathematical expressions
    try:
        # If the statement contains '=', treat it as an equation
        if '=' in statement:
            left, right = statement.split('=')
            return eval(left.strip()) == eval(right.strip())
        # If it's a simple expression, evaluate it
        else:
            result = eval(statement)
            return bool(result)  # Convert to boolean
    except:
        # If evaluation fails, it's not a simple mathematical expression
        pass
