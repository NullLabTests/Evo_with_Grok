...
def tentacle(statement):
    ...
    try:
        # Evaluate the statement and convert the result to a string
        result = str(eval(statement))
        # If the result is 'True' or 'False', convert to boolean
        if result.lower() == 'true':
            return True
        elif result.lower() == 'false':
            return False
        # If the result is not a boolean, return False
        else:
            return False
    except Exception:
...
if __name__ == "__main__":
    ...
    print(tentacle('1 + 1 == 2'))  # Should print: True
    print(tentacle('2 * 2 == 5'))  # Should print: False
    print(tentacle('A and B'))  # Should print: False (as it can't be evaluated)