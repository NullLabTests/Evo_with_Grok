# Domains: text processing, mathematics
def tentacle(expression):
    """
    Evaluate a mathematical expression given as a string.
    
    Args:
    expression (str): A string containing a mathematical expression.
    
    Returns:
    str: The result of the evaluated expression as a string, or an error message if evaluation fails.
    
    Example:
    >>> tentacle('2 + 2')
    '4'
    >>> tentacle('sin(pi/2)')
    '1.0'
    """
    import math
    
    # Define a dictionary of mathematical functions
    math_functions = {
        'sin': math.sin,
        'cos': math.cos,
        'tan': math.tan,
        'sqrt': math.sqrt,
        'log': math.log,
        'exp': math.exp,
        'pi': math.pi,
        'e': math.e
    }
    
    try:
        # Evaluate the expression using the math functions
        result = eval(expression, {"__builtins__": None}, math_functions)
        
        # Convert the result to a string and handle complex numbers
        if isinstance(result, complex):
            return f"{result.real:.6g}{result.imag:+.6g}j"
        else:
            return f"{result:.6g}"
    except Exception as e:
        # Return an error message if evaluation fails
        return f"Error: {str(e)}"

# Test the function
if __name__ == "__main__":
    print(tentacle('2 + 2'))  # Should print: 4
    print(tentacle('sin(pi/2)'))  # Should print: 1
    print(tentacle('sqrt(16)'))  # Should print: 4
    print(tentacle('log(e)'))  # Should print: 1
    print(tentacle('2 ** 3'))  # Should print: 8
    print(tentacle('1/0'))  # Should print: Error: division by zero
    print(tentacle('unknown_function(5)'))  # Should print: Error: name 'unknown_function' is not defined