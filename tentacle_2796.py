...
def tentacle(statement):
    ...
    and check logical equivalences. It first attempts to evaluate the statement
    as a mathematical expression, and if that fails, it checks against a list
    of known logical equivalences.
    
    Args:
    statement (str): A string containing a logical or mathematical statement.
    
    Returns:
    bool or str: True if the logical statement is valid, False if it's not,
                 or a string result if the statement is a valid mathematical expression.
    ...
    '4'
    """
    # First, try to evaluate the statement as a mathematical expression
    try:
        # Evaluate the expression and convert the result to a string
        result = str(eval(statement))
        return result
    except:
        # If evaluation fails, proceed to check logical equivalences
        pass
