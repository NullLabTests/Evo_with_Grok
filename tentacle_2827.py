...
def tentacle(equation):
    ...
    try:
        ...
        if left.startswith('x'):
            ...
            b = 0 if left == 'x' or left == '-x' else float(left[1:]) if left[1] != 'x' else 0
        elif left.endswith('x'):
            a = float(left[:-1]) if left[-2] != 'x' else 1
            b = 0
        else:
            parts = left.split('x')
...
if __name__ == "__main__":
    ...
    print(tentacle('2*x + = 7'))    # Should print: Error: Invalid equation format
    print(tentacle('2*x + 3 = a'))  # Should print: Error: Invalid numerical value in the equation
