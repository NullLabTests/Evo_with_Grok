...
def tentacle(expression):
    """
    Evaluate a mathematical expression given as a string and check if it represents a valid logical statement.
    ...
    str: An error message if evaluation fails or the input is neither a valid mathematical expression nor a logical statement.
    ...
    try:
        # Attempt to evaluate the expression as a mathematical expression
        result = str(eval(expression))
        return result
    except:
        # If evaluation fails, check if it's a logical statement
        statement = expression.lower().strip()
        if statement in logical_equivalences:
            return 'True'
        else:
            return 'False'
