...
def tentacle(statement):
    ...
    # If the statement is not in the list, try to evaluate it as a mathematical expression
    try:
        # Evaluate the statement as a mathematical expression
        result = eval(statement)
        # If the result is a boolean, return it
        if isinstance(result, bool):
            return result
        # If the result is not a boolean, the statement is not a valid logical expression
        return False
    except:
        # If evaluation fails, the statement is not a valid logical expression
        return False
