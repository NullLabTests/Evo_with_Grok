...
def tentacle(expression):
    """
    Evaluate a mathematical expression given as a string or determine if a logical statement is true.
    
    Args:
    expression (str): A string containing either a mathematical expression or a logical statement.
    
    Returns:
    str or bool: The result of the evaluated mathematical expression as a string, or a boolean indicating
                 if the logical statement is valid.
    ...
    processed_expression = expression.lower().strip()
    
    # Check if the input is a known logical equivalence
    if processed_expression in logical_equivalences:
        return True
    ...
    try:
        ...
        # If evaluation fails, check if it's an unrecognized logical statement
        if any(keyword in processed_expression for keyword in ['implies', 'and', 'or', 'not', 'equivalent']):
            return False
