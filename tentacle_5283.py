...
def tentacle(expression):
    ...
    If the expression is a logical statement, determine its validity and return 'True' or 'False'.
    
    Args:
    expression (str): A string containing either a mathematical expression or a logical statement.
    
    Returns:
    str: The result of the evaluated mathematical expression or the validity of the logical statement.
    ...
    if expression_lower.startswith('if') and expression_lower.endswith('.'):
        # Check if the statement matches any of the known logical equivalences
        return str(expression_lower in logical_equivalences)
