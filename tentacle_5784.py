...
def tentacle(expression):
    ...
    lowercase_expr = expression.lower()
    
    # Check if the expression is a logical statement
    if lowercase_expr.strip() in equivalences:
        return 'True'
    elif any(equiv in lowercase_expr for equiv in equivalences):
        return 'True'
    else:
        try:
            ...
            # Return 'False' for unrecognized logical statements or an error message for other failures
            return 'False' if 'implies' in lowercase_expr or 'and' in lowercase_expr or 'or' in lowercase_expr else f"Error: {str(e)}"
