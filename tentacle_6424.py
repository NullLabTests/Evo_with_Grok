...
def tentacle(expression):
    ...
    lowered_expression = expression.lower().strip()
    
    # Check if the expression matches any of the known logical equivalences
    if lowered_expression in equivalences:
        return 'True'
    elif any(equivalence in lowered_expression for equivalence in equivalences):
        return 'True'
    else:
        try:
            ...
            # Return 'False' for unrecognized logical statements or an error message for invalid expressions
            return 'False' if 'implies' in lowered_expression or 'and' in lowered_expression or 'or' in lowered_expression else f"Error: {str(e)}"
