...
def tentacle(statement):
    ...
    # If not found in equivalences, try to evaluate the statement as a mathematical expression
    try:
        # Replace logical operators with their mathematical equivalents
        math_statement = statement.replace("implies", "=>").replace("and", "and").replace("or", "or").replace("not", "not").replace("equivalent to", "==")
        
        # Evaluate the mathematical expression
        result = eval(math_statement)
        
        # If the result is truthy, return True; otherwise, return False
        return bool(result)
    except:
