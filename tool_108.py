...
def tool(
    ...
    Args:
        ...
        split_sentences (bool): If True, return a list of sentences instead of a single string. Default is False.
        remove_stop_words (Optional[List[str]]): A list of stop words to remove from the text. Default is None.
    ...
    if remove_stop_words:
        words = result.split()
        filtered_words = [word for word in words if word.lower() not in remove_stop_words]
        result = ' '.join(filtered_words)

    # Split sentences if specified
    if split_sentences:
