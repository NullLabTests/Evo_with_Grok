...
def tool(
    ...
    Args:
        ...
        stopwords (Optional[List[str]]): List of stopwords to remove. If None and remove_stopwords is True, 
            a default list of English stopwords will be used. Default is None.
        lemmatize (bool): If True, apply lemmatization to the text. Default is False.
        lemmatizer (Optional[callable]): A callable that performs lemmatization. If None and lemmatize is True, 
            a default lemmatizer will be used. Default is None.
    ...
    # Remove punctuation if specified (efficient method from Tool 2)
    ...
    if remove_stopwords:
        if stopwords is None:
            # Use a default list of English stopwords
            stopwords = ['a', 'an', 'the', 'and', 'but', 'or', 'for', 'nor', 'on', 'at', 'to', 'from', 'by', 'with']
        
        words = result.split()
        result = ' '.join(word for word in words if word.lower() not in stopwords)

    # Apply lemmatization if specified
    if lemmatize:
        if lemmatizer is None:
            # Use a simple default lemmatizer (Note: This is a very basic implementation)
            def default_lemmatizer(word):
        ...
        words = result.split()
        result = ' '.join(lemmatizer(word) for word in words)
