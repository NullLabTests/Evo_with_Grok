...
def tool(
    ...
    Args:
        ...
        remove_stop_words (bool): If True, remove common stop words from the text. Default is False.
    ...
    if remove_stop_words:
        stop_words = set(['a', 'an', 'the', 'and', 'but', 'or', 'for', 'nor', 'on', 'at', 'to', 'from', 'with'])
        result = ' '.join(word for word in result.split() if word.lower() not in stop_words)

    # Lemmatize if specified (using a simple approach, actual lemmatization would require a library)
    if lemmatize:
        # This is a very basic lemmatization. For real use, consider using NLTK or spaCy.
        result = ' '.join(
