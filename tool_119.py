...
def tool(
    ...
    Args:
        ...
        preserve_emoji (bool): If True, preserve emojis when removing punctuation or applying case conversion. Default is False.
    ...
    def is_emoji(char: str) -> bool:
        return bool(re.match(r'[\U0001F600-\U0001F64F\U0001F300-\U0001F5FF\U0001F680-\U0001F6FF\U0001F1E0-\U0001F1FF]', char))

    if preserve_formatting:
        # Apply case conversion and replacements while preserving original formatting
        result = ''.join(
            case_func(char) if char.isalpha() and not (preserve_emoji and is_emoji(char)) else 
    ...
    if remove_punctuation:
        if preserve_emoji:
            result = ''.join(char for char in result if char.isalnum() or char.isspace() or is_emoji(char))
        else:
