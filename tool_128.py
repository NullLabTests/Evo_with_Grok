...
def tool(
    ...
    if preserve_formatting:
        # Preserve formatting by applying case conversion to alphabetic characters only
        result = ''.join(
    ...
    if replace_numbers is not None:
        if preserve_formatting:
            # Preserve formatting when replacing numbers
            result = ''.join(
                replace_numbers if char.isdigit() else char
                for char in result
            )
        else:
