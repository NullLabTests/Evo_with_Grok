...
def tool(
    ...
    preserve_emoticons: bool = False,
    replace_urls: Optional[str] = None,
    remove_stopwords: Optional[List[str]] = None
) -> str:
    """
    ...
    Args:
        ...
        preserve_emoticons (bool): If True, preserve emoticons when removing punctuation. Default is False.
        replace_urls (Optional[str]): Replace URLs with this string. If None, keep URLs. Default is None.
        remove_stopwords (Optional[List[str]]): A list of stopwords to remove from the text. Default is None.
    ...
    if replace_urls is not None:
        result = re.sub(r'http\S+', replace_urls, result)

    # Preserve emoticons if specified
    if preserve_emoticons:
        emoticons = re.findall(r'(?::|;|=)(?:-)?(?:\)|\(|D|P)', result)
        result = re.sub(r'(?::|;|=)(?:-)?(?:\)|\(|D|P)', lambda m: f'EMOTICON_{emoticons.index(m.group())}_', result)
    ...
    if remove_punctuation:
        if preserve_emoticons:
            result = re.sub(r'[^\w\sEMOTICON_\d+_]', '', result)
        else:
    ...
    # Restore emoticons if they were preserved
    if preserve_emoticons:
        result = re.sub(r'EMOTICON_(\d+)_', lambda m: emoticons[int(m.group(1))], result)

    # Remove stopwords if specified
    if remove_stopwords:
        words = result.split()
        result = ' '.join(word for word in words if word.lower() not in remove_stopwords)
