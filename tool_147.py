...
def tool(
    ...
    Args:
        ...
        remove_html_tags (bool): If True, remove HTML tags from the text. Default is False.
        apply_url_encoding (bool): If True, apply URL encoding to the text. Default is False.
    ...
    # Apply URL encoding if specified
    if apply_url_encoding:
        result = result.replace(' ', '%20')
        result = ''.join([f'%{ord(char):02X}' if not char.isalnum() else char for char in result])
