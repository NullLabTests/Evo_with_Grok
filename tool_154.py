...
def tool(
    ...
    This function combines and improves upon two existing tools, offering a comprehensive
    text processing solution. It applies transformations in a logical order to ensure
    correct results.
    ...
    Raises:
        ValueError: If an invalid case option is provided.
    """
    ...
    if preserve_formatting:
        # Apply case conversion while preserving original formatting
        result = ''.join(
            case_func(char) if char.isalpha() else char
            for char in result
        )
    else:
