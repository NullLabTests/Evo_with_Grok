...
def tool(input_string, case='lower', remove_punctuation=False, trim_whitespace=True, 
         ...
         preserve_stop_words=None, max_length=None):
    """
    ...
    preserve_formatting (bool): If True, apply case conversion and replacements while preserving original formatting.
                                Default is False.
    preserve_stop_words (list or None): A list of words to preserve in their original form. Default is None.
    max_length (int or None): If specified, truncate the result to this length. Default is None.
    ...
    if preserve_formatting:
        ...
        result = ''.join(
            case_func(char) if char.isalpha() and (preserve_stop_words is None or char.lower() not in preserve_stop_words) else 
    ...
    # Preserve stop words if specified
    if preserve_stop_words:
        words = result.split()
        for i, word in enumerate(words):
            if word.lower() in preserve_stop_words:
                words[i] = input_string.split()[i] if i < len(input_string.split()) else word
        result = ' '.join(words)
