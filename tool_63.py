...
def tool(input_string, case='lower', remove_punctuation=False, trim_whitespace=True, 
         ...
         preserve_acronyms=False, remove_stopwords=False):
    """
    ...
    preserve_formatting (bool): If True, apply case conversion and replacements while preserving original formatting.
                                Default is False.
    preserve_acronyms (bool): If True, preserve acronyms when converting to lowercase. Default is False.
    ...
    if preserve_formatting:
        ...
        # Preserve acronyms when converting to lowercase
        if case == 'lower' and preserve_acronyms:
            result = re.sub(r'\b[A-Z]{2,}\b', lambda m: m.group(0), result)
