...
def tool(
    ...
    keep_common_punctuation: bool = False
) -> str:
    """
    ...
    Args:
        ...
        keep_common_punctuation (bool): If True and remove_punctuation is True, keep common punctuation (.,!?). Default is False.
    ...
    if remove_punctuation:
        if keep_common_punctuation:
            common_punctuation = set(string.punctuation) - set(',.!?')
            result = ''.join(char for char in result if char.isalnum() or char.isspace() or char in ',.!?')
        else:
