...
def tool(
    ...
    preserve_stop_words: bool = False,
    ...
    Args:
        ...
        preserve_stop_words (bool): If True, preserve specified stop words during processing. Default is False.
        stop_words (Optional[set]): A set of stop words to preserve if preserve_stop_words is True. Default is None.
    ...
    if preserve_stop_words and stop_words:
        words = result.split()
        preserved_words = [word if word.lower() in stop_words else word for word in words]
        result = ' '.join(preserved_words)
