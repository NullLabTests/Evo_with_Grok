...
def tool(
    ...
    keep_specific_punctuation: Optional[str] = None
) -> str:
    """
    ...
    Args:
        ...
        keep_specific_punctuation (Optional[str]): A string of punctuation characters to keep if remove_punctuation is True.
            Default is None.
    ...
    # Remove punctuation if specified (using efficient method from Tool 2 with enhancement)
    if remove_punctuation:
        if keep_specific_punctuation:
            result = ''.join(char for char in result if char.isalnum() or char.isspace() or char in keep_specific_punctuation)
        else:
